...
async fn test_echo_command() {
    let output = exec_cmd("echo hello", Some(10), None).await;
...
async fn test_command_with_stderr() {
    let output = exec_cmd("echo error >&2", Some(10), None).await;
    assert_eq!(output.exit_code, Some(0));
...
async fn test_failing_command() {
    let output = exec_cmd("exit 42", Some(10), None).await;
    assert_eq!(output.exit_code, Some(42));
}

#[tokio::test]
async fn test_timeout() {
    let output = exec_cmd("sleep 3", Some(1), None).await;
    assert!(output.timed_out);
...
#[tokio::test]
async fn test_cwd() {
    let output = exec_cmd("pwd", Some(10), Some("/tmp")).await;
    assert_eq!(output.exit_code, Some(0));

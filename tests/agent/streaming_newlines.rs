...
fn test_streaming_with_leading_newlines() {
    ...
    // 验证换行没有被吃掉
    let non_empty_count = streaming_lines
        .iter()
        .filter(|l| {
            let s = format!("{:?}", l);
            !s.trim().is_empty() && s != "\"\""
...
fn test_streaming_with_only_leading_newlines() {
    ...
    assert!(
        has_content,
        "Content 'Based on the file' not found in rendered lines"
    );
}
...
fn test_streaming_full_consistency() {
    ...
    assert_eq!(
        streaming.len(),
        full.len(),
        "streaming and full should produce same number of lines for same input"
    );
}

...
pub fn handle(app: &mut App) -> bool {
    ...
    app.chat_history
        .push(crate::app::ChatEntry::user("/init".to_string()));
    ...
    tokio::spawn(async move {
        ...
        let mut token_usage =
            crate::core::context::token_usage::TokenUsage::with_config(&config_clone);
...
pub fn build_init_prompt(is_update: bool) -> String {
    if is_update {
        let existing_content =
            std::fs::read_to_string(crate::core::agent::preamble::KNOWLEDGE_FILE)
                .unwrap_or_default();
        format!(

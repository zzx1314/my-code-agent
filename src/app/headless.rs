//! Headless mode — WebSocket-driven agent execution without TUI.
...
use crate::core::agent::stream_response::{StreamEvent, StreamResult, stream_response};
use crate::core::context::context_manager::ContextManager;
use crate::core::context::token_usage::TokenUsage;
use crate::core::types::Message;
use crate::core::ws_client::{SessionInfo, WsCommand, WsResponse};
...
pub async fn run_headless(
    ...
    agent: std::sync::Arc<crate::core::agent::preamble::Agent>,
    ...
    let (mut cmd_rx, _shutdown_tx) = crate::core::ws_client::spawn(&config, resp_rx);
...
fn auto_save_session(state: &HeadlessState) {
    let saved_at = std::time::SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    let session_data = crate::core::session::SessionData {
...
fn forward_stream_event(event: &StreamEvent, resp_tx: &mpsc::UnboundedSender<WsResponse>) {
    ...
    match cmd {
        ...
        WsCommand::SwitchSession {
            ...
            let load_result = session_id
                .as_ref()
                .and_then(|sid| crate::core::session::SessionData::load_by_id(sid))
                .or_else(|| {
                    name.as_ref()
                        .and_then(|n| crate::core::session::SessionData::load_by_name(n))
                });
            let display_name = name
                .clone()
                .unwrap_or_else(|| session_id.clone().unwrap_or_default());
            match load_result {
        ...
        WsCommand::DeleteSession {
            ...
            let result = match session_id {
                Some(ref sid) => crate::core::session::SessionData::delete_by_id(sid),
                None => match name {
                    Some(ref n) => crate::core::session::SessionData::delete_by_name(n),

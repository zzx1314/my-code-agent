...
pub struct SessionData {
    ...
    /// Optional unique session ID (generated by the client).
    #[serde(default)]
...
pub struct SessionInfo {
    ...
    pub tokens: u64,
    /// Optional session ID (from SessionData).
    pub id: Option<String>,
}
...
impl SessionData {
    ...
    /// Save the session data to a file named `<id>.json` in the session directory.
    pub fn save_with_id(&self, id: &str) -> Result<(), String> {
        let path = Self::session_file_path(id);
        self.save_to_file(&path)
    }

    /// Load a session by its unique ID from the session directory.
    pub fn load_by_id(id: &str) -> Option<Result<Self, String>> {
        let path = Self::session_file_path(id);
        Self::load_from_file(&path)
    }

    /// Delete a session file by its unique ID.
                        ...
                        if let Some(data) = Self::load_from_file(&path.to_string_lossy()) {
                            if let Ok(data) = data {
                                sessions.push(SessionInfo {
                                    name: data.name.clone().unwrap_or(name_str),
                                    turns: data
                                        .chat_history
                                        .iter()
                        ...
                        if let Ok(session_data) = load_result {
                            ...
                            if !matches.is_empty() {
                                results.push(SearchResult {
                                    session_name: session_data.name.clone().unwrap_or(name_str),

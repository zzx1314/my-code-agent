...
- **file_write**: Create new files on the local filesystem (for editing existing files, use file_update instead). **Important**: When writing to a directory that may not exist (e.g. a new subdirectory), you MUST pass `"create_dirs": true` in the arguments, otherwise the tool will fail with "No such file or directory". **Content size**: For content over ~2000 characters, the tool call arguments may be truncated mid-stream. To avoid this, write a smaller initial version and then use `file_append` to append the remaining content in parts (each under ~2000 characters).
- **file_append**: Append content to the end of a file. If the file does not exist, it will be created. Use this to write large files incrementally — write a smaller initial version with `file_write`, then append additional content in chunks (each chunk under ~2000 characters) using multiple `file_append` calls. This avoids tool call argument truncation that can happen with very large content in a single `file_write` call.
- **file_update**: Edit existing files by specifying a line range to replace lines at a specific position (NOT for appending to the end — use file_append for that). Always read the file first with file_read to see line numbers, then use file_update with `start_line`, `delete_count`, and `new_content` to apply the edit. Set `delete_count=0` to insert, `new_content=""` to delete.
...
- **Insert only**: set `delete_count=0` — inserts new content without removing anything.
  **Note**: For inserting at the END of a file, use `file_append` instead — it's simpler and doesn't require reading the file first to get the line count.

...
mod tests {
    ...
    #[tokio::test]
    async fn test_parse_standard_sse() {
        let data = b"data: {\"choices\":[{\"delta\":{\"content\":\"Hello\"},\"index\":0}]}\n\n";
        let stream = stream::iter(vec![Ok(bytes::Bytes::from_static(data))]);
        let mut chat_stream = ChatStream {
            stream: Box::pin(stream),
            buffer: Vec::new(),
        };
        
        let result = chat_stream.next().await.unwrap().unwrap();
        assert_eq!(result.choices[0].delta.content.as_deref(), Some("Hello"));
    }

    #[test]
    fn test_parse_next_chunk_multiple_events() {
        let mut chat_stream = ChatStream {
            stream: Box::pin(stream::empty()),
            buffer: b"data: {\"choices\":[{\"delta\":{\"content\":\"Hello\"},\"index\":0}]}\n\ndata: {\"choices\":[{\"delta\":{\"content\":\" World\"},\"index\":0}]}\n\n".to_vec(),
        };
        
        let result1 = chat_stream.parse_next_chunk().unwrap();
        assert_eq!(result1, "{\"choices\":[{\"delta\":{\"content\":\"Hello\"},\"index\":0}]}");
        
        let result2 = chat_stream.parse_next_chunk().unwrap();
        assert_eq!(result2, "{\"choices\":[{\"delta\":{\"content\":\" World\"},\"index\":0}]}");
        
        assert!(chat_stream.parse_next_chunk().is_none());
    }

    #[tokio::test]
    async fn test_parse_sse_with_extra_lines() {
        let data = b"data: {\"choices\":[{\"delta\":{\"content\":\"Hello\"},\"index\":0}]}\nextra line\n\n";
        let stream = stream::iter(vec![Ok(bytes::Bytes::from_static(data))]);
        let mut chat_stream = ChatStream {
            stream: Box::pin(stream),
    ...
    #[tokio::test]
    async fn test_parse_trailing_sse() {
        let data = b"data: {\"choices\":[{\"delta\":{\"content\":\"Hello\"},\"index\":0}]}";
        let stream = stream::iter(vec![Ok(bytes::Bytes::from_static(data))]);
        let mut chat_stream = ChatStream {
            stream: Box::pin(stream),
    ...
    #[tokio::test]
    async fn test_parse_trailing_sse_with_extra_lines() {
        let data = b"data: {\"choices\":[{\"delta\":{\"content\":\"Hello\"},\"index\":0}]}\nextra line";
        let stream = stream::iter(vec![Ok(bytes::Bytes::from_static(data))]);
        let mut chat_stream = ChatStream {
            stream: Box::pin(stream),
